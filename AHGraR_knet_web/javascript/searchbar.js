// Load list of available projects
 ...
 if (node_data.length == 0){
   // Small cheat - we change the name of the results tree if we have no results
   var jsdata = {'core': {'data': [{'id': "$$$all_res", "parent":"#", "text": "No results found!"}],
   'themes': {'variant':'large', 'icons':false}, 'animation':false}, 'plugins':["checkbox", "sort"]};
   document.getElementById('render_data_btn').style.visibility = 'hidden';
   document.getElementById('render_tree_btn').style.visibility = 'hidden';
 } else {
   var jsdata = {'core': {'data': [{'id': "$$$all_res", "parent":"#", "text": "Search results"}],
   'themes': {'variant':'large', 'icons':false}, 'animation':false}, 'plugins':["checkbox", "sort"]};
   document.getElementById('render_data_btn').style.visibility = 'visible';
   document.getElementById('render_tree_btn').style.visibility = 'visible';
 }
 ...
 function renderTree()
 {         
   if($('#jstree_div li.jstree-open').length) {
 ...
 function renderJSON()
 {
   ...
   for (var i = 0, len = selected_gene_data.length; i < len; i++){
     ...
     if (selected_node_data){
       selected_genes.push(selected_node_data)
     };
   };
   // Did the user select any results?
